...
std::u32string CWCUtils::UTF8ToUnicode(const std::string& str)
{
    if (str.empty())
    {
        return {};
    ...
    for (size_t i = 0; i < str.size();)
    {
        ...
        else if ((c & 0xE0) == 0xC0) // 110x-xxxx 10xx-xxxx
        {
            ...
            wideChar |= (char32_t(str[i + 1]) & 0x3F);
            i += 2;
        }
        else if ((c & 0xF0) == 0xE0) // 1110-xxxx 10xx-xxxx 10xx-xxxx
        {
            ...
            wideChar |= (char32_t(str[i + 1]) & 0x3F) << 6;
            wideChar |= (char32_t(str[i + 2]) & 0x3F);
            i += 3;
        }
        else if ((c & 0xF8) == 0xF0) // 1111-0xxx 10xx-xxxx 10xx-xxxx 10xx-xxxx
        {
            ...
            wideChar |= (char32_t(str[i + 1]) & 0x3F) << 12;
            wideChar |= (char32_t(str[i + 2]) & 0x3F) << 6;
            wideChar |= (char32_t(str[i + 3]) & 0x3F);
            i += 4;
        }
        else // 1111-10xx 10xx-xxxx 10xx-xxxx 10xx-xxxx 10xx-xxxx
        {
            ...
            wideChar |= (char32_t(str[i + 1]) & 0x3F) << 18;
            wideChar |= (char32_t(str[i + 2]) & 0x3F) << 12;
            wideChar |= (char32_t(str[i + 3]) & 0x3F) << 6;
            wideChar |= (char32_t(str[i + 4]) & 0x3F);
            i += 4;
        }
        u32str.push_back(wideChar);
    }

    return u32str;
}

typedef struct _tag_width_interval
{
    int start;
    int end;
} WIDTH_INTERVAL;

static WIDTH_INTERVAL ZERO_WIDTH[] = {
...
static WIDTH_INTERVAL WIDE_EASTASIAN[] = {
...
static bool intable(WIDTH_INTERVAL* table, int table_length, char32_t c)
{
...
uint32_t CWCUtils::GetWideCharWidth(char32_t ucs)
{
    if (ucs == 0 || ucs == 0x034F || (0x200B <= ucs && ucs <= 0x200F) || ucs == 0x2028 || ucs == 0x2029 ||
        (0x202A <= ucs && ucs <= 0x202E) || (0x2060 <= ucs && ucs <= 0x2063))
    {
        return 0;
    }
    ...
    if (intable(ZERO_WIDTH, sizeof(ZERO_WIDTH) / sizeof(WIDTH_INTERVAL), ucs))
    {
        return 0;
    }

    return intable(WIDE_EASTASIAN, sizeof(WIDE_EASTASIAN) / sizeof(WIDTH_INTERVAL), ucs) ? 2 : 1;
}
