...
void Parser<EventListener>::parseFragment(const std::string& data)
{
...
template <typename EventListener>
void Parser<EventListener>::processOnceViaStateMachine(uint8_t ch)
{
...
template <typename EventListener>
auto Parser<EventListener>::parseBulkText(char const* begin, char const* end) noexcept
    -> std::tuple<ProcessKind, size_t>
{
    ...
    return {ProcessKind::ContinueBulk, 0};
}

template <typename EventListener>
void Parser<EventListener>::handle(ActionClass actionClass,
                                                      Action action,
                                                      uint8_t codepoint)
{
    (void) actionClass;
